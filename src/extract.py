...
class Extract:
    ...
    def __init__(self, extract_id, material_id, bookmark, path, 
                 review_date, due_date, number_of_reviews, 
                 interval_to_next_review, a_factor, priority_percentage,
                 is_suspended):
        ...
        # TODO When creating a new extract, check if this file already exists and rename
        if path == '':
            instant = datetime.datetime.now()
            instant_str = str(instant.year)+str(instant.month)+str(instant.day)+"-"+\
                str(instant.hour)+str(instant.minute)+str(instant.second)
